...
def convert(
    ...
    # The intrinsic box keeps the artwork's own proportions. Every asset logo in this archive is
    # square and gets exactly ``size_dp`` both ways as before; a wordmark is not — OneRoyal's is
    # 564×53 — and a square intrinsic box on a 10:1 drawing is not a smaller logo, it is a squashed
    # one. Rounded to a tenth of a dp, which is finer than a density bucket can resolve.
    intrinsic_width = round(size_dp * width / height, 1) if height else size_dp
    ...
    lines = [
        ...
        f'    android:width="{intrinsic_width:g}dp"',
