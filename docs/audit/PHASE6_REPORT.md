...
| Download-size check in CI | `scripts/release/check-bundle-size.sh`, workflow step *Check the download size a phone would pay for* | bundletool builds the split APK set from the signed AAB and `get-size total` reports the largest per-device download. First measurement, 4.33.0: **13.7–13.9 MiB per device** (the universal APK is 17 MB). The gate is set at 16 MiB so it holds the line from here; the audit's 9 MiB needs the help catalogue and future imagery in an on-demand pack, which Phase 2 could not do offline. |
