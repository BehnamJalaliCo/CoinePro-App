...
## ۱. مهم‌ترین یافته: سیگنال‌ها پشت اشتراک‌اند — روی هر دو

این با تصمیمِ محصول در تضاد است («سیگنال و کپی‌ترید و بقیه برای کاربر عادی کاملاً
باز باشد») و با هیچ تغییری سمت کلاینت حل نمی‌شود.
...
`_has_membership` عمداً **fail-closed** است. و چون `vip_members.user_id` کلید خارجی
به `users(user_id)` دارد، حسابِ ایمیل‌محور ساختاراً عضو نمی‌شود — یعنی کاربری که
در اپ ثبت‌نام می‌کند **هرگز** هیچ سیگنالی نمی‌بیند.

خبر خوب: یک متغیر محیطی است. `MOBILE_SIGNALS_REQUIRE_VIP=0` فهرست را برای هر
کاربر واردشده باز می‌کند. بقیهٔ مسیرهای موبایل TradeYar هیچ گیتِ اشتراکی ندارند.
...
`/public/signals/active` و `/public/signals/recent` هر دو `require_vip` دارند و
**فلگی برای خاموش کردنش نیست**. فقط `/public/signals/showcase` باز است و عمداً یک
سیگنالِ **بسته‌شده** می‌دهد.
