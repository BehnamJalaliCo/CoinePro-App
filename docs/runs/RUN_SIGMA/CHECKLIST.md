...
## RUN Τ — the flick, and the pencil

| Item | State | Evidence | Frame |
|---|---|---|---|
| **1** The chart travels when it is thrown | ✅ | **The diagnosis in the report was wrong about the cause and right about the symptom, and the difference is worth stating.** The three suspects were a velocity divided by density, a tracker reading predicted events, and a stop threshold. `ChartFlingRegressionTest` drives a synthetic 3 000 px/s flick through the *real* event stream and the chart covers 97 % of what the decay curve predicts — so nothing divides by density, and `MotionEventPredictor` is not in this path at all (it belongs to freehand drawing). What was wrong was the **curve**: friction was 3.8 per second, and a flick covers `v / f`, so a 4 300 px/s release covered 1 130 px — one screen — where TradingView covered 2 900. Friction is now 1.25 and the cut-off 240 px/s, which is exactly two pixels a frame at 120 Hz and is what ends the creep the report filmed. The tracker is also fed every sample the digitiser took rather than one a frame, which costs nothing and is strictly closer to the finger | — **a gesture, so not a still.** `ChartFlingRegressionTest` (2) through the whole page, `ChartFlingTest` (4) on the curve, `ChartPixelsTest` on the JVM twin. How it *feels* is owed to the owner's three flicks |
| **2** The pencil is back, in every mode and both orientations | ✅ | One condition: the draw button was hidden whenever `showsAdvancedChrome` was false, which is what «تازه‌کارم» sets — so a beginner in portrait had neither the band's pencil nor the tool column, while landscape, which builds its strip elsewhere, kept it. **Simple mode changes defaults, not the toolbar** (the owner's decision, overriding Ω3). `ChartToolbarTest` (6) holds three modes × both orientations: the band's four other buttons always, and the drawing tools reachable exactly once — the pencil, or the permanent column that deliberately replaces it on a wide window | `tau-toolbar-simple-portrait-fa.png`, `tau-toolbar-simple-landscape-fa.png` |

### What run Τ does not claim

**The brief's own acceptance numbers cannot all hold at once, and the ones kept are the distances.**
A flick covers `(v − cut-off) / f` and lasts `ln(v / cut-off) / f`: asking for more travel *and* a
shorter stop is asking one number to move both ways. At 1.25 a 3 000 px/s flick covers two screens
and takes two seconds, against the brief's «1.5 screens, stop within 1.4 s». TradingView — the thing
being matched — takes two seconds itself.

**Nothing here was felt.** A container renders the event stream faithfully and drops no frames; what
it cannot tell anybody is whether the chart now moves like the one in the other app. That is the
owner's three flicks on BTCUSDT H1, and it is the only thing that closes this.

## RUN Τ2 — the empty box, the market count, and the swipe

| Item | State | Evidence | Frame |
|---|---|---|---|
| **1** The box beside the timeframe shows the instrument | ✅ | **A layout bug, and the test that would have missed it.** The ticker was in the tree the whole time — an assertion that it *exists* passed on the broken build. Its measured size was `210 x 1`: the wheel lays out five rows of eighteen points inside a pill of thirty-six, so a `Column` measured against the pill gave the first row eighteen, the second eighteen, and the third — the one carrying the instrument — one pixel. `requiredHeight` takes the column out from under the pill's constraint, which is what a wheel is, and the `Box` clips it back. Two smaller things came with it: `SymbolNeighbours.isEmpty` meant «nowhere to turn» and was being read as «nothing to draw», so a ring of one hid the cell entirely — it is `cannotTurn` now, and the pill's only reason to disappear is having no name; and the cell grows to 104 points for a long ticker, because eighty cut `DOGEUSDT` to «DOGEUSD», which reads as a different instrument rather than a clipped one | `tau2-wheel-ring-fa.png`, `tau2-wheel-one-fa.png`, `tau2-wheel-ring-dark-fa.png`. `SymbolWheelBandTest` (3) asserts the ticker's **height**, not its presence — presence was never the question |
| **2** LBank: every market whose logo we have | ✅ measured, ❌ to grow without new artwork | **The app is already doing this, and the number is the artwork's.** `scripts/quality/report-market-coverage.py` asks LBank and counts: 1 333 USDT pairs listed, 1 007 traded in the last day, and this repository holds a mark for **178** of them — which is the 189 on screen once the other quotes are counted. Those 178 carry **50.4 %** of LBank's 24-hour turnover. The rest are leveraged tokens (`ADA3L`), tokenised equities (`AAPLX`) and micro-caps, and **not one of them has a vector in any of the three archives** — `binance-icons` (563), `tv-logos` (689), `crypto-icons` (483) — so there is nothing to convert. The remote `/assets/logo/<SYMBOL>.webp` fallback answers 404 for every one I tried | — **a count, not a picture.** The script prints it, and prints the top uncovered markets by turnover so the next hundred marks can be sourced in the order that buys the most book |
| **3** The swipe is as smooth as TradingView's | ❌ **owed to a device** | `ChartPanCostProbeTest` times two kinds of drag frame on the same chart: one that crosses a bar — which recomposes and invalidates every layer — and one that moves the picture inside a bar, which only redraws. **93.9 ms against 98.3 ms, a ratio of 1.05.** So the per-frame cost is *not* the recomposition and *not* the invalidation; and the rest of that number is this container rasterising a million pixels in software, which tells nobody anything about a phone. I am not going to refactor the chart's hottest path on a guess — that is exactly what run Τ's first diagnosis got wrong. This needs a Perfetto or Macrobenchmark trace from the owner's phone | — **a cost, so not a still.** The probe is repeatable and its number is in the report |

## RUN Υ — the chart that stopped, and the study that would not leave

The owner reported two things in one message, and they turned out to be the same chart: a study
switched on, and then everything that went wrong around it.

| Item | State | Evidence | Frame |
|---|---|---|---|
| **1** The chart stops responding while the reader swipes left and right | ✅ for the cost that could be measured, ⏳ for how it feels | **Four explanations were measured and three of them were wrong**, which is the point of measuring. Not the fling clock (`withFrameNanos` into `ChartFling`, whose curve is in nanoseconds — the millisecond twin is `KineticScroll`, which only the JVM tests drive); a 3 000 px/s flick is delivered over **99 frames**, biggest step 4 bars of 164. Not the derive: every study over 50 000 bars costs **27 ms**, once per page, and it is linear. Not a dropped gesture: a flick onto a chart still coasting carries **164 bars against 164** from rest. What *was* wrong is one line in the draw pass. `SignalMarkers.thin` runs **once a frame** whenever the bars are closer than six points — which is the zoom a reader looking through history is at — and it built a `HashMap` of **every bar the chart holds** to find each mark's index. Both its inputs grow with use and neither grows with what is on screen: a structure study draws a mark a bar, and flicking back pages the series towards the 50 000-bar resident ceiling. Measured on a 50 000-bar chart carrying 200 marks: **5 348 µs and 476 KiB of garbage, per frame, on the drawing thread.** The index is a binary search over `CandleSeries.time` now — it is sorted, because the type refuses to be built otherwise — and the draw pass takes the **window first** and thins only what is on the plot. Same figures after: **13 µs and 4 KiB**, and flat in history depth | — **a cost, so not a still.** `SignalMarkerThinningCostTest` (5) carries the before and after and the unchanged answer; `ChartFlingRegressionTest` (4) and `ChartHistoryDepthProbeTest` carry the three explanations that were ruled out. Whether the phone now keeps up is the owner's swipe |
| **2** A study can be taken off the chart it was put on | ✅ | **Seven of the catalogue's studies had no legend row at all.** The legend addresses a study by *where its row sits* — «the fourth line on the price scale», «the second strip» — and `sr`, `supplydemand`, `autofib`, `swings`, `fractals`, `chopzone` and a `correlation` with no second instrument have neither: their whole output is levels, or marks, or nothing yet. So there was no ×, no gear and no eye for any of them, and the only way back was to find the row again in a catalogue of eighty-three. That is the report, exactly: «ضرب در روی صفحه چارت رو می‌زنم ولی هنوز اندیکاتوره هست». They get a row of their own now, carrying the study's **id** rather than a position — `ChartLegendTarget.Study` — because a position is the thing they do not have. `ChartDerived` also keeps an owner beside every level and every mark, which is what makes the eye on such a row mean anything | `upsilon-study-row-open-phone-fa.png` beside `upsilon-study-row-removed-phone-fa.png` — the `S/R ×2` line on the candles and «حمایت و مقاومت» in the legend with an eye, a gear and a ×, then the same chart with both gone. The first is not drawn from a decoration the test wrote: the study is switched on through a **real controller**, the disclosure and the × are **real taps on the real legend**, and the × is asserted to report `Study("sr")`. `IndicatorRemovalTest` (4) then sweeps the **whole catalogue** — every study that draws something must answer the row it draws — and `ChartLegendRowsTest` (3) holds the row's shape |

### What run Υ does not claim

**Item 1 is a fix for a cost, not a verdict on a feel.** What was found is real, it is in the hottest
path there is, and it grew with exactly the use the owner described. What nobody in this container
can say is whether removing it is *enough* — the rest of a drag frame is this machine rasterising a
million pixels in software. The Perfetto trace run Τ2 asked for is still the thing that closes it,
and item 3 of Τ2 stays ❌ until it arrives.
