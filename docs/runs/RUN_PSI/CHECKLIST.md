...
| 16 | The service description the reader agrees to | ✅ **asked, answered, and the answer was «nothing changes»** | The row shipped ❌ on purpose: `docs/legal/TERMS.md` §6-3 describes automatic copy trading **on LBank** — the crypto venue, not forex — and only the owner could say whether that service had gone too. They have: «کپی ترید البنک میماند». So the passages stand, un-edited, and the asymmetry is correct rather than left over — the app carries no copy trading and the terms describe some, because **the terms are the service's and not this binary's**. `BLOCKED.md §Ψ16` carries the answer and is closed | — **a decision, and the evidence is that the diff is empty.** `sync-legal-documents.py --check` passes on the unchanged documents |
