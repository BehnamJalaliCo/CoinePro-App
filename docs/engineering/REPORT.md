...
| 4 | NamaScript on Expanded: editor ≥ 400, code\|chart 50/50 toggle, no soft wrap, line numbers, minimap, keep the tabs | **done**: the panel's own floor is the 400 the editor needs. The code field sits in a `horizontalScroll`, which gives it unbounded width and so **stops it wrapping** — `softWrap = false` is not on the `TextFieldValue` overload, and a wrapped line in a language with 90-character `ta.*` calls is an editor you cannot read a stack trace against. Line numbers were already there; the minimap is a 24-point `Canvas` drawing one bar per line at 60 % of its length. The split is decided by **the pane's own width**, not the window's: `showsTwoPanes` answers a question about the screen, and this screen is often a 400-point side panel, where a fifty-fifty split leaves the editor two hundred points and `plot(rsi, ti` on a line. Two columns of `EDITOR_MIN` or the split is not offered at all | `theScriptScreenSplitsWhereThereIsRoom` — the toggle is there on a 1478-point tablet (`run-h-script-split-en-dark`, code beside chart); `chartAndScriptEnDark` — it is **absent** in the docked panel, which gets the whole 480 points for the editor (`run-h-chart-script-en-dark`, `run-h-chart-script-fa-dark`); `CodeFieldTest` |
...
### RUN I — a script is an indicator on the main chart (4.73.0)

The owner's question was «چرا استودیو یک چارت جداگانه دارد؟», and the answer in the code was that the
studio's two-hundred-bar preview had become the **destination**. TradingView's Pine editor has the
same preview and it also has «Add to chart», and everything a reader actually wants from a script
they wrote happens after that button. This run is that button and everything behind it.

| # | asked | done | proof |
| --- | --- | --- | --- |
| 0.1 | A compiled script is an instance in the same registry as a built-in, and every legend/eye/gear/×/reorder/merge/z-order behaviour works unchanged | **done**, and the design is one sentence: a script instance is addressed by an **indicator id** — `ChartScript.ownerId`, `"nama:1"`, a string exactly as `"ema"` is a string. Every field on `ChartUiState` that keys by indicator id therefore addresses it with no second code path: `hiddenIndicators` is its eye, `indicatorColours`/`indicatorWidths` its style, `paneOrder`/`paneMerges`/`separated` its place. `toggleIndicator` routes a script id to `removeScript`, which is the legend's × | `ChartScriptsTest` (9 cases); `RunIProofTest.aScriptSitsInTheLegendBesideTheBuiltIns` — `overlays` holds one built-in line and one script line and `shownOverlayOwners` is `[ema, nama:1]`, which **is** the legend's own list; `run-i-script-on-chart-en-dark` |
| 0.2 | «Add to chart» in the studio beside Run, on every library card, and a Custom tab in the indicator sheet; on a tablet split the right half is the **main chart** | **done**: a full-width primary button above Run (`putScript`, which hot-swaps rather than stacking a second copy of the same name), a button on every saved script, strategy and preset card, and a «My scripts» section at the **top** of the indicator sheet — above the eighty-three built-ins, because these are the studies this reader wrote. The split's other half is `ChartMirror`, the reader's own controller drawn read-only; the sandbox is one chip away | `run-i-studio-add-to-chart-en-dark` with «Add to chart» asserted present and unpressed; `theIndicatorSheetListsTheReadersScripts` — adding the same script twice leaves **one** instance; `run-i-indicator-sheet-custom-en-dark` |
| 0.3 | Live binding: every Run hot-swaps and keeps inputs, colours and position; a compile error never removes the last good version and shows a red dot; a budget breach pauses that instance only | **done**: `ChartScriptEngine` keeps an `IncrementalRunner` and the **last good result** per instance, so a reader who types a bracket keeps the study they were reading and the legend grows a dot (`CoineProChart.warningSeries` → `ChartLegendRow.warning`, drawn in the sell colour before the swatch). E401/E406/E407 — the node budget, the clock and the retained-series budget — become a `ChartScriptPause` with a line number and the words «متوقف شد — این اسکریپت برای اجرا روی هر کندل کند است (خط N)» | `ChartScriptsTest.a script that stops compiling keeps the drawing it had`, `…is paused rather than reported as wrong` |
| 0.4 | A settings sheet generated from `input.*`, identical to a built-in's; per-instance style; numbered duplicates | **done**: `ScriptSettingsSheet` draws the same three tabs — Inputs · Style · Visibility — with the inputs **discovered by running the script** rather than read from a fixed table, which is the one thing that could not be shared with `IndicatorSettingsSheet`. Style and Visibility are the same behaviours because they key by the same id. A second copy of a script is «RSI Zones (2)» | `theScriptsInputsBecomeItsSettings` — one declared input, named, and setting it is remembered on the instance; `a second copy of a script is numbered rather than renamed`; `run-i-script-inputs-en-dark` |
| 0.5 | Instances saved in templates and layouts, restored on cold start, per pane in a multi-chart layout, and a «Restore script» placeholder for a missing one | **done**: field 24 of `SymbolChartState` and field 13 of `ChartLayout`, both through `ChartScriptCodec` — **the source travels**, Base64'd. Storing an id would have restored a chart with a hole in it the first time somebody renamed the script it pointed at, and the hole would be silent. A controller is one symbol, so a four-chart layout already keeps each pane's instances apart. A row whose source did not decode comes back as a placeholder that draws nothing rather than vanishing | `ChartScriptCodecTest` — a script containing the store's own three separators, and one containing this codec's, both survive; one corrupt row does not take the others |
| 0.6 | `alertcondition` entries in the alert sheet for that symbol/TF; `strategy.*` markers on the main chart; the strategy report from the gear | **done** for the alerts and the markers: `AlertTrigger.ScriptCondition` carries the source and the condition name (Base64, because a script contains whatever the reader typed), `GuestAlertMarketSource` runs the distinct scripts once per pass **against the candles it already loads for indicator alerts** and reports whether each condition held on the last closed bar, and `AlertConditions` routes that in as 1 or 0 — the same shape an indicator alert uses. The alert reads «نمااسکریپت: RSI Zones ← Oversold cross», fires once per closed bar, and a script that cannot be run produces `NaN` and fires nothing. `strategy.*` markers and the report already arrive through `ScriptResult` and are carried per instance in `ChartScriptDraw.strategies` | `AlertTriggerCodecTest` — a script whose condition name contains `;` and `\|` round-trips, and the trigger fires only where the routing says the condition held; `theScriptsAlertConditionIsCarried`; `run-i-script-alerts-en-dark` |
| 0.7 | Incremental evaluation, off the UI thread, committed atomically | **done**: `refreshScripts` launches on the controller's `workers` dispatcher and swaps one `ChartScriptDraw` in, so the overlays, the panes and their owners can never be read half-changed. The watch re-runs on the **identity** of the visible series — bars arriving, a timeframe change, a replay step — and deliberately not on a pan, because a script is a function of the bars and re-running one per frame of a drag is what would make a charted script unusable | the evaluation's own numbers still want a device; see below |
| 0.8 | Legend reads `ScriptName params · value`; the script's colours; `plotshape` scales; `bgcolor` behind volume | **done** through the existing renderer, which is the point: a script's line is a `ChartLine` and its strip a `ChartPane`, so it is laid out by the same code that lays out RSI. `table` is the one clause with nothing behind it — NamaScript v1.1 has no `table.*`, so there is no table to place; the rule is written where it will be read when one lands | `run-i-script-pane-en-dark` — «RSI Zones» named in the legend, its own pane on its own 17.5–79.0 scale with the `hline(70)` dashed across it |
| 4 | Snippet chips and every studio label from resources; `TabletEnglishTest` extended to the studio | **done**, and the leak was larger than the chips: the ten **shipped presets** were Persian-only, and a preset is not a label — tapping one inserts a script whose `plot(title = "تند")` becomes a row in the reader's own legend. `ScriptPresetsEn` is the English table, the code identical line for line and only the prose inside the quotes moved | `ScriptPresetsEnTest` — every preset has an English twin, no English preset carries Arabic script, **both languages of all ten compile and run**, and the two are the same code once comments and string contents are removed; `TabletEnglishTest.theScriptStudioOnATabletIsEnglishThroughout` |
| 5 | A 60 % `surface0` scrim behind the legend rows | **done**: `LEGEND_PLATE_ALPHA` 0 → **0.60**. The number has moved twice and the note at the constant says why: 0.82 was a panel, 0 was TradingView's (which works because its top-left is usually empty), and a chart carrying a reader's own scripts is not usually empty | the constant, and every re-recorded golden |

**Items 1, 2 and 3 are recordings, and this environment has no device.** There is no `/dev/kvm`, no
emulator image and no `emulator` binary in the SDK here, so neither a Gradle Managed Device nor
`adb emu screenrecord` can run. The *code* each of those items describes shipped in earlier runs and
is unchanged: the drawing interaction — haptic, 8 dp handles, the floating toolbar, the magnifier,
magnet — in run E (4.67.0); the motion — `MotionScheme.expressive()`, springs for everything
spatial, predictive back, the tick flash, rolling digits — in sprint D; the physics — decay fling,
rubber-band, focal pinch, axis pinch, double-tap reset, the auto-scale spring, history load without
a jump — in runs B and 4.60.0. The **grep item 2 asks for is a gate rather than a report**:
`check-motion-policy.sh` fails the build on a `tween(` on the same line as any spatial transition,
which is the machine-checked form of «zero interactive tween».

**What a device would still settle**, unchanged and now with one more line: the fling benchmark with
three script indicators on the chart (0.7's p95 ≤ 8 ms phone / ≤ 12 ms tablet), the 120 fps
...
- I. NamaScript on the main chart — done (4.73.0): a compiled script is an indicator addressed by
  an indicator id, so the legend, the eye, the gear, the ×, the pane order, the colour and the
  width all work on it with no second code path; «Add to chart» in the studio and on every library
  card; a settings sheet generated from the script's own inputs; instances saved with the symbol
  and with a layout, source and all; `alertcondition` as a real alert trigger the background worker
  can evaluate; the ten presets translated, code included.
