...
اپ اندروید `Pro Chart` (مخزن `BehnamJalaliCo/CoinePro-App`) از نسخه‌ی 4.42.0 دو تغییر داد که به
سمت شما مربوط است:
...
```
GET {BASE}/api/v1/public/market/depth?symbol=BTCUSDT&limit=20
```

با همان شکل پاسخ روت عضو. سند قدیمی: `docs/SERVER_ASKS_DOM.md`. اجباری نیست؛ fallback کار می‌کند.

---

## کار ۵ — پین‌های TLS (اولویت: متوسط، هماهنگی لازم)
