...
| **layouts, watchlists, drawings synced** | one workspace | the watchlist sync already exists (`WatchlistSyncController`, GET/PUT); the drawing sync store exists (`DrawingSyncStore`); the chart layout (`ChartLayoutStore`, `ChartWorkspaceStore`) needs the same GET/PUT pair — a JSON document per account with a version number and a `409` that carries the winning document, which is what the watchlist does |
