...
| the drawables | **1,150 vector XML files, all generated** from SVG by `scripts/design/svg-to-vector.py`. **1,117 still have their SVG source in the repo, including all 226 with `aapt:attr` gradients.** 33 are hand-authored XML with no source — see W1½ |
...
**There is nothing to convert, and that was measured on 2026-09-21 rather than assumed.** The
1,150 vector XML files in `core/designsystem/src/main/res/drawable/` are *generated*: each opens
with `<!-- Generated by scripts/design/svg-to-vector.py from design/… -->`. The **SVG sources are
in this repository** — 5,021 of them — and **1,117 of the 1,150 still have theirs, including all
226 of the ones with `aapt:attr` gradients** that would otherwise have needed flattening.

So:

1. **The browser reads the SVGs; Android keeps the generated XML.** Compose Multiplatform reads
   SVG on every target. Do not convert the 226, do not flatten a gradient, do not invent a second
   drawing of anything.
2. **Extend `scripts/design/svg-to-vector.py`, or add a sibling beside it**, to emit the second
   output: the SVGs a `composeResources/drawable/` tree needs, under the same names the XML uses,
   so `ChartIcons.kt`'s mapping stays one mapping. Idempotent and re-runnable — the icon set grows.
3. **The 33 that have no SVG source are the whole of the remaining work.** They are hand-authored
   XML: `tv_tool_eraser`, `tv_zoom_in`, `tv_chart_footprint` and a handful of other chart-type
   glyphs. List them, convert them once to SVG, and check the SVG in as the source so the next
   person finds it where the other 1,117 are.
4. **Prove it visually.** The golden screenshot tests already render the tool rail, the indicator
   picker and the watchlist. Android's frames must be **byte-identical** — nothing about the phone
   changed — and the browser's are the new ones.

**Acceptance:** every gate green, the Android golden frames unchanged, a `scripts/` entry that can
be run again, and the 33 named with their new sources committed.
