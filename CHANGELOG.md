...
### Added
- **`StudioProofTest`**: four frames of the studio in `docs/qa/screenshots/4.66/` — the
  completion strip with signatures, a diagnostic with its line and column and code, the console
  and the strategy card, the generated inputs — each asserting the thing it shows is on screen.
- **`docs/qa/NAMASCRIPT_CONFORMANCE.md`**: the 380 conformance scripts indexed with their
  expectations, generated by `scripts/quality/gen_conformance_index.py`.
- **The platform pin-set**: `network_security_config.xml` carries the same nine digests and the
  same expiry as OkHttp's list, so the terminal's WebView is pinned too; `NetworkSecurityPinsTest`
  holds the two lists equal.
- **`docs/qa/DEVICE_PROOFS.md`**: the exact commands for the fling benchmark, the jank count, the
  120 fps recording and the on-device NamaScript timings; **`NamaScriptDevicePerfTest`**
  (instrumentation) prints compile / evaluate / realtime for the 303-line script over 20 000 bars.

### Changed
- The code field opens with the caret at the end of the source, where a reader continues a
  script, so the completion strip is live at once.
...
grid were already here; this release gives the activity a resize contract, teaches the app about a
hinge, turns sheets into dialogs on a wide window, and generates the parity matrix from the tests
rather than typing it.

### Added
- **Fold posture.** `androidx.window` 1.5.0; `CoineProFold` (table-top, book, hinge position)
  provided from `MainActivity`. In table-top the chart keeps its plot above the crease.
- **Sheets on a tablet** open as a centred dialog no wider than 560 dp instead of a strip across
  the glass. Same body, same close; the phone is unchanged.
- **`docs/qa/PARITY_MATRIX.md`**, generated by `scripts/quality/gen_parity_matrix.py` from the
  render tests' qualifiers and checked by the consistency gate; **`docs/qa/INPUT_MATRIX.md`**
  (touch, stylus, mouse, keyboard, trackpad, per chart action); **`docs/qa/RTL_TABLET.md`** (rail
  on the right, list-detail mirrored, price scale stays right — a decision, written down).
- **Nine tablet goldens**: explore, ideas and chart in portrait; watchlist, explore, ideas, menu
  and chart in landscape.
- **Tests**: `NavigationParityTest` (rail = bar), `ChartKeyboardTest` (every shortcut, down-only),
  `SheetShapeTest`, `LayoutParityArchitectureTest` (a workbench reaches no state its screen lacks;
  no state holder lives only behind a window-class branch), `CoineProFoldTest`, `ChartFoldTest`.

### Changed
- `MainActivity` is `resizeableActivity` and handles every configuration change it can survive,
  so a split-screen resize no longer restarts the chart.
- `Modifier.chartShortcuts` is public so the keyboard map can be exercised from a test.

### Not done, and said so
- `material3-adaptive` is not added (offline cache; `CoineProWindowClass` covers the decisions).
- Home grid, DOM beside the chart, screener table, docked right panels, list-detail drag divider,
  device soak/benchmark/recording. See `docs/engineering/REPORT.md` §4.
