...
object ScriptLessons {
    ...
    val ALL: List<ScriptLesson> = listOf(
        ...
        ScriptLesson(
            ...
            body = listOf(
                "این مهم‌ترین ایدهٔ زبان است. «close» یک عدد نیست؛ قیمت بسته‌شدن همهٔ کندل‌ها با هم است. وقتی می‌نویسید «close * 2»، این محاسبه روی تک‌تک کندل‌ها انجام می‌شود و نتیجه‌اش دوباره یک سری است.",
                "پس در نما اسکریپت حلقه (for) وجود ندارد و لازم هم نیست: هر عبارتی که می‌نویسید یک‌بار برای کل نمودار نوشته می‌شود و روی همهٔ کندل‌ها اجرا می‌شود.",
                "اگر یک طرف محاسبه عدد ثابت باشد، همان عدد برای همهٔ کندل‌ها تکرار می‌شود. «close - 10» یعنی از قیمت هر کندل ده واحد کم کن.",
            ),
            example = """
                spread = high - low
                plot(spread, title = "دامنهٔ کندل", color = color.blue, pane = "separate")
            """.trimIndent(),
            takeaway = "یک‌بار می‌نویسید، برای همهٔ کندل‌ها حساب می‌شود.",
        ),
        ScriptLesson(
            id = "absent",
        ...
        ScriptLesson(
            ...
            body = listOf(
                ...
                "«close > close[1]» یعنی این کندل بالاتر از کندل قبلی بسته شده — و مثل هر عبارت دیگری، برای همهٔ کندل‌ها حساب می‌شود.",
        ...
        ScriptLesson(
            ...
            example = """
                ...
                above = close > trend
                plot(iff(above, high, low), title = "لبهٔ همسو", color = color.teal)
            """.trimIndent(),
        ...
        ScriptLesson(
            ...
            body = listOf(
                "همهٔ اندیکاتورها زیر «ta.» هستند: «ta.sma»، «ta.ema»، «ta.rsi»، «ta.atr»، «ta.macd» و بقیه. فهرست کاملشان در برگهٔ «مرجع» است.",
            ...
            """.trimIndent(),
            takeaway = "«ta.» همان اندیکاتورهای خود برنامه است، نه نسخهٔ دیگری از آنها.",
        ),
        ScriptLesson(
            id = "crosses",
            title = "تقاطع‌ها",
            body = listOf(
                "«ta.crossover(a, b)» فقط روی همان کندلی درست است که a از پایین به بالای b رفته — نه روی همهٔ کندل‌هایی که a بالاتر است.",
        ...
        ScriptLesson(
            ...
            body = listOf(
                ...
                "همهٔ خطوطی که پنل جدا می‌خواهند در یک پنل جمع می‌شوند و یک مقیاس مشترک دارند — که دقیقاً همان چیزی است که مقایسه‌شان را ممکن می‌کند.",
        ...
        ScriptLesson(
            ...
            body = listOf(
                ...
                "«title» اسم آن ورودی در پنل است. «min» و «max» بازه‌اش را می‌بندند، و همین بستن است که جلوی «دورهٔ صفر» یا «دورهٔ منفی» را می‌گیرد.",
                "مقداری که خواننده انتخاب کرده ذخیره می‌شود و دفعهٔ بعد هم همان است. اگر بعداً بازه را در کد تنگ‌تر کنید، مقدار ذخیره‌شده به داخل بازهٔ جدید کشیده می‌شود، نه اینکه اسکریپت خطا بدهد.",
            ),
            example = """
                length = input(14, title = "دورهٔ میانگین", min = 2, max = 200)
