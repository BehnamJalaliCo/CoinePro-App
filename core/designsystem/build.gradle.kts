...
dependencies {
    ...
    // The index-to-country table, and since 4.71.0 the bilingual name tables behind `SymbolText`.
    // `core:symbols` is plain Kotlin with no Compose and no Android resources — it depends on
    // `core:model` and nothing else — so this direction introduces no cycle, and the alternative
    // was a second copy of the table living next to the drawables. `api`, not `implementation`:
    // `SymbolMeta` is the receiver of `localName()`/`localRowName()`, so every screen that names
