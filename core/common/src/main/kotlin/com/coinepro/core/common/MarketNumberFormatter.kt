 ...
 *
 * Every figure goes through [NumberStyle], which pins the locale: prices, quantities and
 * percentages stay Latin-digit and dot-decimal so a trader can compare them against MetaTrader,
...
object MarketNumberFormatter {
    fun price(value: Double, decimals: Int = 2): String {
        return BidiText.isolateLtr(NumberStyle.grouped(value, decimals))
    }
    ...
    fun signedPercent(value: Double): String {
        ...
        return BidiText.isolateLtr(sign + NumberStyle.percent(value, 2))
    }
    ...
    fun money(
        ...
        val magnitude = NumberStyle.grouped(kotlin.math.abs(value), decimals)
        val sign = when {
    ...
    fun quantity(value: Double, unit: String, decimals: Int = 4): String {
        return BidiText.isolateLtr("${NumberStyle.grouped(value, decimals)} $unit")
    }
}
