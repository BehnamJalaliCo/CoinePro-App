...
class RoomCandleCache(
    ...
    override suspend fun read(symbol: String, timeframe: Timeframe, limit: Int): List<OhlcBar> =
        runCatching {
            dao.newest(symbol.uppercase(), timeframe.wire, limit.coerceAtLeast(1))
    ...
    override suspend fun write(symbol: String, timeframe: Timeframe, bars: List<OhlcBar>) {
        ...
        runCatching {
            dao.replace(
                candles = bars.mapNotNull { bar -> bar.toEntity(key, timeframe.wire, at) },
                symbol = key,
 ...
 */
private fun OhlcBar.toEntity(symbol: String, timeframe: String, at: Long): CachedCandleEntity? {
    if (t <= 0L) return null
