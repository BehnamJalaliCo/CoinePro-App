...
object TimeScale {
     ...
     */
    fun boundaryOf(time: Long, previous: Long, zone: ChartZone): TimeTickUnit? {
        val offsetNow = zone.offsetSeconds(time)
        val offsetBefore = zone.offsetSeconds(previous)
    ...
    fun ticks(
        ...
        val startOffset = zone.offsetSeconds(times[first])
        val endOffset = zone.offsetSeconds(times[last])
        ...
        for (index in maxOf(first, 1)..last) {
            ...
            val offset = fixed ?: zone.offsetSeconds(time)
            val before = fixed ?: zone.offsetSeconds(previous)
     ...
     * year are read off [CivilDate.ofEpochDay], which is arithmetic on the day number the division
     * already produced.
     */
    private fun boundaryOfLocal(local: Long, localBefore: Long): TimeTickUnit? {
        val day = local.floorDiv(SECONDS_PER_DAY)
        val dayBefore = localBefore.floorDiv(SECONDS_PER_DAY)
        if (day != dayBefore) {
            val date = CivilDate.ofEpochDay(day)
            val before = CivilDate.ofEpochDay(dayBefore)
            return when {
                date.year != before.year -> TimeTickUnit.YEAR
                date.month != before.month -> TimeTickUnit.MONTH
                ...
                (day + WEEK_ANCHOR).floorDiv(DAYS_PER_WEEK) !=
                    (dayBefore + WEEK_ANCHOR).floorDiv(DAYS_PER_WEEK) -> TimeTickUnit.WEEK
                else -> TimeTickUnit.DAY
            }
        }
        if (local.floorDiv(SECONDS_PER_HOUR) != localBefore.floorDiv(SECONDS_PER_HOUR)) {
            return TimeTickUnit.HOUR
        }
        if (local.floorDiv(SECONDS_PER_MINUTE) != localBefore.floorDiv(SECONDS_PER_MINUTE)) {
