...
class ChartTimeZoneTest {
    ...
    fun `a month boundary is decided in the zone it is asked for`() {
        assertEquals(
            ...
            TimeTickUnit.MONTH,
            TimeScale.boundaryOf(afterMidnight, beforeMidnight, tehran.asChartZone()),
        )
        assertEquals(
            "and in UTC the same pair of bars are both still March, so it is only a new hour",
            TimeTickUnit.HOUR,
            TimeScale.boundaryOf(afterMidnight, beforeMidnight, utc.asChartZone()),
        )
    }
    ...
    fun `two bars in the same month are a week rather than a month`() {
        ...
        assertEquals(TimeTickUnit.WEEK, TimeScale.boundaryOf(later, earlier, tehran.asChartZone()))
        assertEquals(TimeTickUnit.WEEK, TimeScale.boundaryOf(later, earlier, utc.asChartZone()))
    }
    ...
    fun `a year turning over outranks the month it also turns over`() {
        ...
        assertEquals(TimeTickUnit.YEAR, TimeScale.boundaryOf(january, december, tehran.asChartZone()))
    }

    @Test
    fun `two bars inside one minute open nothing at all`() {
        val moment = ZonedDateTime.of(2026, 4, 10, 9, 0, 10, 0, tehran).toEpochSecond()
        assertNull(TimeScale.boundaryOf(moment + 5, moment, tehran.asChartZone()))
    }
